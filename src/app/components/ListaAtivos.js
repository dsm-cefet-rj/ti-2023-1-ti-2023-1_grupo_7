...
const ListaAtivos = (props) => {
    ...
    function __getAtivos(){
        ...
        (<div className="lista">{ativos.filter((element)=>{return element.tipo===props.tipo}).map((element,i)=><Ativo key={i} data={{...element,qnt:carteiraAtual.ativos[carteiraAtual.ativos.map((a)=>a.id).indexOf(element.id)].quantidade}} deleteAtivo={event=>deleteAtivo(element.id)}/>)}</div>);
    }
